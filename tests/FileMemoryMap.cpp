...
BOOST_AUTO_TEST_CASE(open_close)
{
    ...
    BOOST_CHECK(!map.isMapped());
    BOOST_CHECK(!map);
    BOOST_CHECK_EQUAL(map.fileSize(), 0);
    const char data[] = "hello world";
    map.truncate(sizeof(data));
    BOOST_CHECK_EQUAL(map.fileSize(), sizeof(data));
    ...
    BOOST_CHECK(map.isOpened());
    BOOST_CHECK(!map.isMapped());
    map.mapFull(PROT_READ, MAP_PRIVATE, 1);
    BOOST_CHECK(map.isMapped());
    BOOST_CHECK_EQUAL(reinterpret_cast<const char *>(map.data()), data + 1);
    map.unmap();
    BOOST_CHECK(!map.isMapped());
    map.mapPart(4, PROT_READ, MAP_PRIVATE, 1);
    BOOST_CHECK(map.isMapped());
    BOOST_CHECK(memcmp(map.data(), data + 1, 4) == 0);
    map.close();
    BOOST_CHECK(!map.isOpened());
    BOOST_CHECK(!map.isMapped());
}
